...
const initialDestinations: Destination[] = [
  ...
  },
  {
    ...
    streamKey: "FB-xxxxxxxxxxxxxxxxxxxx",
    title: "Church Facebook Page",
    enabled: true,
  },
  {
    id: "3",
    ...
    streamKey: "IG-xxxxxxxxxxxxxxxxxxxx",
  ...
  },
  {
    id: "4",
    platform: "Owncast",
    rtmpUrl: "rtmp://stream.ourchurch.org/live",
    streamKey: "owncast-secret-key-xxxxx",
    title: "Church Website Stream",
    enabled: true,
  },
]
...
export default function DestinationsPage() {
    ...
    const fetchDestinations = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Fallback to initial data if API unavailable
  ...
  const toggleEnabled = useCallback(async (id: string) => {
    ...
    try {
      ...
      setDestinations((prev) =>
        prev.map((d) => (d.id === id ? { ...d, enabled: newEnabled } : d))
      )
  ...
  const saveEditing = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${apiUrl}/api/destinations/${editForm.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const deleteDestination = useCallback(async (id: string) => {
    ...
    try {
      await fetch(`${apiUrl}/api/destinations/${id}`, { method: "DELETE" })
    } catch {
      // Continue anyway
    }
  ...
  const addDestination = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${apiUrl}/api/destinations`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...newDestination, id }),
      })
      ...
      if (response.ok) {
        ...
        setDestinations((prev) => [...prev, { ...newDestination, id }])
      }
    } catch {
      setDestinations((prev) => [...prev, { ...newDestination, id }])
    }
